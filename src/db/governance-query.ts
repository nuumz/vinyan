...
export function normalizeGovernanceQuery(
  input: GovernanceTraceQuery,
): Required<Pick<GovernanceTraceQuery, 'limit' | 'offset'>> & Omit<GovernanceTraceQuery, 'limit' | 'offset'> {
...
export function formatReplayForCLI(summary: DecisionReplaySummary): string {
  ...
  if (summary.evidenceObservedAt != null) lines.push(`Evidence Seen:   ${formatTimestamp(summary.evidenceObservedAt)}`);
  ...
  if (summary.goalGrounding && summary.goalGrounding.length > 0) {
    ...
    for (const gg of summary.goalGrounding) {
      lines.push(
        `  - phase=${gg.phase} action=${gg.action} drift=${gg.goalDrift} freshnessDowngraded=${gg.freshnessDowngraded}`,
      );
