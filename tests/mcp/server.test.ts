...
import type { HypothesisTuple, OracleVerdict, Fact } from "../../src/core/types.ts";
...
describe("vinyan_ast_verify", () => {
  test("calls ast oracle correctly", async () => {
    ...
    const response = await server.handleRequest(
      makeToolCallRequest("vinyan_ast_verify", validHypothesis),
    );
...
describe("vinyan_type_check", () => {
  test("calls type oracle correctly", async () => {
    const { server, oracle } = makeServer();
    const response = await server.handleRequest(
      makeToolCallRequest("vinyan_type_check", validHypothesis),
    );
...
describe("vinyan_blast_radius", () => {
  test("calls dep oracle correctly", async () => {
    const { server, oracle } = makeServer();
    const response = await server.handleRequest(
      makeToolCallRequest("vinyan_blast_radius", validHypothesis),
    );
...
describe("vinyan_query_facts", () => {
  test("returns facts for known target", async () => {
    const { server } = makeServer();
    const response = await server.handleRequest(
      makeToolCallRequest("vinyan_query_facts", { target: "src/app.ts" }),
    );
...
describe("error handling", () => {
  test("unknown tool returns error", async () => {
    const { server } = makeServer();
    const response = await server.handleRequest(
      makeToolCallRequest("nonexistent_tool", {}),
    );
