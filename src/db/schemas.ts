...
const RuleActionSchema = z.enum([
  "escalate", "require-oracle", "prefer-model", "adjust-threshold", "assign-worker",
]);

const RuleStatusSchema = z.enum(["probation", "active", "retired"]);

export const EvolutionaryRuleRowSchema = z.object({
  id: z.string(),
  source: z.string(),
...
export const WorkerProfileRowSchema = z.object({
  ...
  tool_allowlist: z.string().nullable().optional().transform(v => v ? JSON.parse(v) as string[] : undefined),
