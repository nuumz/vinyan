...
const ruleArb: fc.Arbitrary<EvolutionaryRule> = fc.record({
  ...
  condition: fc.record({
    filePattern: fc.option(fc.string(), { nil: undefined }),
    oracleName: fc.option(fc.string(), { nil: undefined }),
    riskAbove: fc.option(fc.float({ min: 0, max: 1, noNaN: true }), { nil: undefined }),
    modelPattern: fc.option(fc.string(), { nil: undefined }),
  }),
  action: actionArb,
...
describe('PBT: computeBlastRadius BFS completeness', () => {
  ...
  const graphArb = fc
    .array(fc.tuple(fileArb, fc.array(fileArb, { minLength: 0, maxLength: 3 })), { minLength: 0, maxLength: 10 })
    .map((edges) => {
