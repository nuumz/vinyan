...
describe('LatencyBudgetsSchema cross-field validation (WU6)', () => {
  ...
  test('strictly ordered latency budgets pass', () => {
    const result = VinyanConfigSchema.safeParse(configWithLatency({ l0: 100, l1: 2000, l2: 10000, l3: 60000 }));
    expect(result.success).toBe(true);
  });

  test('l0 > l1 fails: order l0=5000, l1=2000, l2=10000, l3=60000', () => {
    const result = VinyanConfigSchema.safeParse(configWithLatency({ l0: 5000, l1: 2000, l2: 10000, l3: 60000 }));
  ...
  test('l1 > l2 fails: order l0=100, l1=15000, l2=10000, l3=60000', () => {
    const result = VinyanConfigSchema.safeParse(configWithLatency({ l0: 100, l1: 15000, l2: 10000, l3: 60000 }));
    expect(result.success).toBe(false);
  });

  test('l2 > l3 fails: order l0=100, l1=2000, l2=70000, l3=60000', () => {
    const result = VinyanConfigSchema.safeParse(configWithLatency({ l0: 100, l1: 2000, l2: 70000, l3: 60000 }));
    expect(result.success).toBe(false);
  });

  test('equal adjacent values fail (not strictly ordered): l0=l1=2000', () => {
    const result = VinyanConfigSchema.safeParse(configWithLatency({ l0: 2000, l1: 2000, l2: 10000, l3: 60000 }));
    expect(result.success).toBe(false);
  });
});
