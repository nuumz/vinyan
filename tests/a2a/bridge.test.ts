...
describe("A2ABridge", () => {
  describe("tasks/send", () => {
    test("creates TaskInput with source 'a2a'", async () => {
      ...
      await bridge.handleRequest(makeJsonRpcRequest("tasks/send", {
    ...
    test("returns A2A Task with completed status", async () => {
      ...
      const response = await bridge.handleRequest(makeJsonRpcRequest("tasks/send", {
  ...
  describe("tasks/get", () => {
    test("returns task status after send", async () => {
      ...
      // Then get its status
      const response = await bridge.handleRequest(makeJsonRpcRequest("tasks/get", {
        id: "get-test-task",
      }));
  ...
  describe("tasks/cancel", () => {
    test("returns cancelled status", async () => {
      ...
      // Then cancel
      const response = await bridge.handleRequest(makeJsonRpcRequest("tasks/cancel", {
        id: "cancel-test-task",
      }));
