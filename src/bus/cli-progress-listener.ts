...
export function attachCLIProgressListener(
  bus: VinyanBus,
  ...
  detachers.push(bus.on("task:start", ({ input, routing }) => {
    write(`${dim("[vinyan]")} Starting task ${bold(input.id)} at L${routing.level} with ${routing.model ?? "no model"}`);
  }));

  detachers.push(bus.on("worker:dispatch", ({ taskId, routing }) => {
    if (verbose) {
      write(`${dim("[vinyan]")} Dispatching ${taskId} to worker ${dim(`(L${routing.level})`)}`);
    }
  }));

  detachers.push(bus.on("oracle:verdict", ({ oracleName, verdict }) => {
    if (verbose) {
  ...
  detachers.push(bus.on("task:escalate", ({ fromLevel, toLevel, reason }) => {
    write(`${dim("[vinyan]")} ${yellow("Escalating")} L${fromLevel} → L${toLevel}: ${reason}`);
  }));

  detachers.push(bus.on("task:timeout", ({ taskId, elapsed_ms, budget_ms }) => {
    write(`${dim("[vinyan]")} ${red("TIMEOUT")} task ${taskId} after ${elapsed_ms}ms (budget: ${budget_ms}ms)`);
  }));

  detachers.push(bus.on("trace:record", ({ trace }) => {
    const quality = trace.qualityScore
      ? ` ${dim(`(quality: ${trace.qualityScore.composite.toFixed(2)})`)}`
      : "";
    const outcome = trace.outcome === "success" ? green(trace.outcome) : red(trace.outcome);
    write(`${dim("[vinyan]")} Attempt: ${outcome}${quality}`);
  }));

  detachers.push(bus.on("task:complete", ({ result }) => {
