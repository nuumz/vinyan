...
import type { TaskInput, PerceptualHierarchy, WorkingMemoryState } from "../../src/orchestrator/types.ts";
...
const VALID_DAG_JSON = JSON.stringify({
  nodes: [
    ...
    { id: "n2", description: "update db layer", targetFiles: ["src/db.ts"], dependencies: ["n1"], assignedOracles: ["type", "dep"] },
  ],
});
...
describe("TaskDecomposerImpl", () => {
  ...
  test("failed approaches included in memory feedback", async () => {
    ...
    const memory: WorkingMemoryState = {
      failedApproaches: [
        { approach: "direct edit", oracleVerdict: "type errors", timestamp: Date.now() },
      ],
      activeHypotheses: [],
      unresolvedUncertainties: [],
      scopedFacts: [],
