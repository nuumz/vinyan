...
import { runGate, type GateRequest, type GateVerdict } from "../../src/gate/index.ts";
...
describe("Risk-Tiered Gate Pipeline (G2)", () => {
  ...
  test("medium riskScore (0.2-0.4) runs structural oracles but skips test oracle", async () => {
    const verdict = await runGate(makeRequest({
      riskScore: 0.3,
      params: { file_path: "math.ts", workspace, content: "export function add(a: number, b: number): number { return a + b; }" },
    }));
    ...
    const structuralOracles = Object.keys(verdict.oracle_results).filter(
      name => ["type", "lint", "dep"].includes(name),
    );
