...
`,
  ...
  test('function-signature: correct param count', () => {
    const verdict = verify(makeHypothesis('function-signature', { functionName: 'greet', paramCount: 2 }));
    expect(verdict.verified).toBe(true);
    expect(verdict.evidence).toHaveLength(1);
  });

  test('function-signature: wrong param count', () => {
    const verdict = verify(makeHypothesis('function-signature', { functionName: 'greet', paramCount: 3 }));
    expect(verdict.verified).toBe(false);
  ...
  test('function-signature: wrong param names', () => {
    const verdict = verify(makeHypothesis('function-signature', { functionName: 'greet', params: ['user', 'msg'] }));
    expect(verdict.verified).toBe(false);
    expect(verdict.reason).toContain('Param name mismatch');
  });

  test('function-signature: function not found', () => {
    const verdict = verify(makeHypothesis('function-signature', { functionName: 'missing', paramCount: 1 }));
    expect(verdict.verified).toBe(false);
    expect(verdict.reason).toContain('not found');
  });
