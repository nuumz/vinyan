...
import { TRACE_SCHEMA_SQL, MODEL_PARAMS_SCHEMA_SQL, SELF_MODEL_PARAMS_SCHEMA_SQL } from "../../src/db/trace-schema.ts";
import type { TaskInput, PerceptualHierarchy, ExecutionTrace, SelfModelPrediction } from "../../src/orchestrator/types.ts";
    ...
    test("basis remains static-heuristic if accuracy < 0.4 even with many observations", async () => {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        model.calibrate(pred, {
          ...
          duration_ms: 50000, // very different from predicted
