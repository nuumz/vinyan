...
export class MigrationRunner {
  ...
  migrate(db: Database, migrations: Migration[], options?: { dryRun?: boolean }): MigrateResult {
    ...
    const pending = sorted.filter((m) => m.version > currentVersion).map((m) => m.version);
    ...
    const finalVersion = applied.length > 0 ? (applied[applied.length - 1] ?? currentVersion) : currentVersion;
