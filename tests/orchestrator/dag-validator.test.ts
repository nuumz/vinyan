...
import { validateDAG, allCriteriaMet, formatFailures } from "../../src/orchestrator/dag-validator.ts";
...
describe("validateDAG", () => {
  ...
  test("single-node DAG passes all criteria", () => {
    const dag: TaskDAG = {
      nodes: [{ id: "n1", description: "do stuff", targetFiles: ["a.ts"], dependencies: [], assignedOracles: ["type"] }],
    };
  ...
  test("C1/C4: dependency references unknown node → caught by C4", () => {
    const dag: TaskDAG = {
      nodes: [
        { id: "n1", description: "a", targetFiles: ["a.ts"], dependencies: ["n99"], assignedOracles: ["type"] },
      ],
    };
  ...
  test("C3 pass: empty blast radius", () => {
    const dag: TaskDAG = {
      nodes: [
        { id: "n1", description: "a", targetFiles: [], dependencies: [], assignedOracles: ["type"] },
      ],
    };
  ...
  test("C4 fail: self-cycle", () => {
    const dag: TaskDAG = {
      nodes: [
        { id: "n1", description: "a", targetFiles: ["a.ts"], dependencies: ["n1"], assignedOracles: ["type"] },
      ],
    };
    expect(validateDAG(dag, []).valid_dependency_order).toBe(false);
  });
  ...
  test("C4 fail: unknown dependency ID", () => {
    const dag: TaskDAG = {
      nodes: [
        { id: "n1", description: "a", targetFiles: ["a.ts"], dependencies: ["n999"], assignedOracles: ["type"] },
      ],
    };
    expect(validateDAG(dag, []).valid_dependency_order).toBe(false);
  });
  ...
  test("allCriteriaMet true when all pass", () => {
    const criteria = { no_orphans: true, no_scope_overlap: true, coverage: true, valid_dependency_order: true, verification_specified: true };
    expect(allCriteriaMet(criteria)).toBe(true);
  });

  test("allCriteriaMet false when any fail", () => {
    const criteria = { no_orphans: true, no_scope_overlap: false, coverage: true, valid_dependency_order: true, verification_specified: true };
    expect(allCriteriaMet(criteria)).toBe(false);
  });

  test("formatFailures returns messages for failed criteria only", () => {
    const criteria = { no_orphans: true, no_scope_overlap: false, coverage: false, valid_dependency_order: true, verification_specified: true };
