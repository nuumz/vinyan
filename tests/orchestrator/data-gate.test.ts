...
import { checkAllDataGates, checkDataGate } from '../../src/orchestrator/data-gate.ts';
...
describe('checkDataGate', () => {
  ...
  test('sleep_cycle gate satisfied when thresholds met', () => {
    const gate = checkDataGate('sleep_cycle', makeStats({ traceCount: 100, distinctTaskTypes: 5 }), DEFAULT_THRESHOLDS);
    expect(gate.satisfied).toBe(true);
  });

  test('sleep_cycle gate not satisfied when only traces met', () => {
    const gate = checkDataGate('sleep_cycle', makeStats({ traceCount: 200, distinctTaskTypes: 3 }), DEFAULT_THRESHOLDS);
    expect(gate.satisfied).toBe(false);
  });
  ...
  test('conditions include current values and thresholds', () => {
    const gate = checkDataGate('sleep_cycle', makeStats({ traceCount: 50, distinctTaskTypes: 2 }), DEFAULT_THRESHOLDS);
