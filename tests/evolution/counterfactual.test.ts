...
import {
  ...
  summarizeByTaskType,
} from "../../src/evolution/counterfactual.ts";
...
describe("PH3.6: Counterfactual Analysis", () => {
  describe("buildQualityLookup", () => {
    test("groups by task type and routing level", () => {
      const traces = [
        makeTrace({ task_type_signature: "refactor::ts", routingLevel: 1, qualityScore: { composite: 0.6, architecturalCompliance: 0.6, efficiency: 0.6, dimensions_available: 2, phase: "phase0" } }),
        makeTrace({ task_type_signature: "refactor::ts", routingLevel: 1, qualityScore: { composite: 0.8, architecturalCompliance: 0.8, efficiency: 0.8, dimensions_available: 2, phase: "phase0" } }),
        makeTrace({ task_type_signature: "refactor::ts", routingLevel: 2, qualityScore: { composite: 0.9, architecturalCompliance: 0.9, efficiency: 0.9, dimensions_available: 2, phase: "phase0" } }),
      ];
    ...
    test("skips traces without quality scores", () => {
      const traces = [
        ...
        makeTrace({ qualityScore: { composite: 0.8, architecturalCompliance: 0.8, efficiency: 0.8, dimensions_available: 2, phase: "phase0" } }),
      ];
  ...
  describe("analyzeCounterfactuals", () => {
    test("identifies traces that would benefit from routing up", () => {
      ...
      for (let i = 0; i < 5; i++) {
        traces.push(makeTrace({
          id: `l1-${i}`,
          routingLevel: 1,
          qualityScore: { composite: 0.5, architecturalCompliance: 0.5, efficiency: 0.5, dimensions_available: 2, phase: "phase0" },
        }));
      }
      ...
      for (let i = 0; i < 5; i++) {
        traces.push(makeTrace({
          id: `l2-${i}`,
          routingLevel: 2,
          qualityScore: { composite: 0.9, architecturalCompliance: 0.9, efficiency: 0.9, dimensions_available: 2, phase: "phase0" },
        }));
      }
    ...
    test("skips L3 traces (already max level)", () => {
      const traces = [
        makeTrace({ routingLevel: 3 as RoutingLevel }),
      ];
      const lookup = buildQualityLookup(traces);
