...
import type { ExtractedPattern, ExecutionTrace } from "../../src/orchestrator/types.ts";
...
function makeTrace(id: string, workerId: string, opts: {
...
describe("generateRule for worker-performance patterns", () => {
  ...
  test("does not extract file_pattern from fingerprint-format signature", () => {
    const pattern = makeWorkerPattern({ taskTypeSignature: "refactor::.ts::small" });
    const rule = generateRule(pattern);
    // Worker assignment rules don't use file_pattern — signatures are fingerprint keys
    expect(rule!.condition.file_pattern).toBeUndefined();
  });
  ...
  test("includes oracle_name in condition when present", () => {
    const pattern = makeWorkerPattern({ oracleName: "ast" });
    const rule = generateRule(pattern);
    expect(rule!.condition.oracle_name).toBe("ast");
...
describe("backtestWorkerAssignment", () => {
  ...
  test("returns fail for non-assign-worker rule", () => {
    const pattern: ExtractedPattern = {
      id: "ap-001", type: "anti-pattern", description: "test",
      frequency: 10, confidence: 0.8, taskTypeSignature: "refactor::.ts::small",
      approach: "x", sourceTraceIds: [], createdAt: Date.now(), decayWeight: 1.0,
    };
    const rule = generateRule(pattern)!;
  ...
  test("passes when assigned worker has better quality", () => {
    ...
    for (let i = 0; i < 6; i++) {
      traces.push(makeTrace(`w1-train-${i}`, "w1", { quality: 0.9, timestamp: baseTime + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    for (let i = 0; i < 4; i++) {
      traces.push(makeTrace(`w2-train-${i}`, "w2", { quality: 0.5, timestamp: baseTime + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    // Validation: w1 still better
    for (let i = 0; i < 3; i++) {
      traces.push(makeTrace(`w1-val-${i}`, "w1", { quality: 0.85, timestamp: baseTime + 50000 + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    for (let i = 0; i < 2; i++) {
      traces.push(makeTrace(`w2-val-${i}`, "w2", { quality: 0.4, timestamp: baseTime + 50000 + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
  ...
  test("fails when assigned worker has worse quality", () => {
    ...
    // Training
    for (let i = 0; i < 6; i++) {
      traces.push(makeTrace(`w1-${i}`, "w1", { quality: 0.3, timestamp: baseTime + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    for (let i = 0; i < 4; i++) {
      traces.push(makeTrace(`w2-${i}`, "w2", { quality: 0.9, timestamp: baseTime + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    // Validation: w1 still worse
    for (let i = 0; i < 3; i++) {
      traces.push(makeTrace(`w1-v-${i}`, "w1", { quality: 0.3, timestamp: baseTime + 50000 + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
    for (let i = 0; i < 2; i++) {
      traces.push(makeTrace(`w2-v-${i}`, "w2", { quality: 0.9, timestamp: baseTime + 50000 + i * 1000, affectedFiles: ["src/foo.ts"] }));
    }
