...
import { analyzeSessionDir, analyzeSessionFile, formatMetrics, type SessionMetrics } from '../../src/gate/analyzer.ts';
...
describe('Session Analyzer', () => {
  ...
  test('analyzes single session file correctly', () => {
    ...
    const entries: SessionLogEntry[] = [
      makeEntry({ decision: 'allow', durationMs: 100 }),
      makeEntry({ decision: 'block', reasons: ['Oracle "type" rejected: type error'], durationMs: 200 }),
      makeEntry({ decision: 'allow', durationMs: 150 }),
    ];
