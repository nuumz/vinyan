...
describe("Phase 0 Compatibility Guards", () => {
    ...
    const gateWithVerdicts = {
      verify: async () => {
        const verdicts: Record<string, import("../../src/core/types.ts").OracleVerdict> = {
          "ast-oracle": { verified: true, type: "known", confidence: 1.0, evidence: [], fileHashes: {}, duration_ms: 1 },
          "dep-oracle": { verified: true, type: "known", confidence: 1.0, evidence: [], fileHashes: {}, duration_ms: 1 },
        };
  ...
  test("completed result has valid TaskResult shape per Phase 0 contract", async () => {
    ...
    expect(typeof result.trace.duration_ms).toBe("number");
  });
