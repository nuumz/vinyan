...
export class SessionStore {
  ...
  insertSession(session: SessionRow): void {
    this.db.run(
      `INSERT INTO session_store (id, source, created_at, status, working_memory_json, compaction_json, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [session.id, session.source, session.created_at, session.status,
       session.working_memory_json, session.compaction_json, session.updated_at],
    );
  }
  ...
  updateSessionStatus(id: string, status: SessionRow["status"]): void {
    this.db.run(
      "UPDATE session_store SET status = ?, updated_at = ? WHERE id = ?",
      [status, Date.now(), id],
    );
  }
  ...
  updateSessionCompaction(id: string, compactionJson: string): void {
    this.db.run(
      "UPDATE session_store SET compaction_json = ?, status = 'compacted', updated_at = ? WHERE id = ?",
      [compactionJson, Date.now(), id],
    );
  }

  updateSessionMemory(id: string, memoryJson: string): void {
    this.db.run(
      "UPDATE session_store SET working_memory_json = ?, updated_at = ? WHERE id = ?",
      [memoryJson, Date.now(), id],
    );
  }

  listActiveSessions(): SessionRow[] {
    return this.db.query("SELECT * FROM session_store WHERE status = 'active' ORDER BY created_at DESC").all() as SessionRow[];
  }

  listSuspendedSessions(): SessionRow[] {
    return this.db.query("SELECT * FROM session_store WHERE status = 'suspended' ORDER BY created_at DESC").all() as SessionRow[];
  }
  ...
  insertTask(task: SessionTaskRow): void {
    this.db.run(
      `INSERT INTO session_tasks (session_id, task_id, task_input_json, status, result_json, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [task.session_id, task.task_id, task.task_input_json, task.status,
       task.result_json, task.created_at],
    );
  }
  ...
  getTask(sessionId: string, taskId: string): SessionTaskRow | undefined {
    ...
    ).get(sessionId, taskId) as SessionTaskRow | undefined;
  }

  updateTaskStatus(sessionId: string, taskId: string, status: SessionTaskRow["status"], resultJson?: string): void {
    this.db.run(
      "UPDATE session_tasks SET status = ?, result_json = ? WHERE session_id = ? AND task_id = ?",
      [status, resultJson ?? null, sessionId, taskId],
    );
  }

  listSessionTasks(sessionId: string): SessionTaskRow[] {
    return this.db.query(
      "SELECT * FROM session_tasks WHERE session_id = ? ORDER BY created_at ASC",
  ...
  listPendingTasks(): SessionTaskRow[] {
    return this.db.query(
      "SELECT * FROM session_tasks WHERE status IN ('pending', 'running') ORDER BY created_at ASC",
