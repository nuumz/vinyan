...
> **Cross-reference**: [concept.md](../foundation/concept.md) §1 (vision), [decisions.md](../architecture/decisions.md) §1 (thesis), [theory.md](../foundation/theory.md) §3 (theory), [implementation-plan.md](../design/implementation-plan.md) (phased roadmap).
...
> **Expert Panel Review (April 2026):** Questions 10-22 below were identified by the expert panel review. See [expert-review.md](../analysis/expert-review.md) for full context and [gap-analysis.md](../analysis/gap-analysis.md) §10.6 for severity mapping.

| # | Question | Current Assumption | Impact if Wrong | Status |
|---|----------|-------------------|-----------------|--------|
| 10 | Should Critic fail-closed (`approved:false`) or fail-uncertain on LLM error? | Fail-open (`approved:true` with `confidence:0.3`) | Silent removal of semantic verification at L2+, A2 violation | ⚠️ **Open** — Expert consensus: fail-closed is strictly safer. Source: `llm-critic-impl.ts:192-201` |
| 11 | What is the minimum state for Working Memory crash checkpoint? | No checkpoint — WorkingMemory is in-process only | Crash mid-task loses all failure evidence, retry loops rediscover same failures | ⚠️ **Open** — Proposed minimum: `{phase, routingLevel, retryCount, workingMemory}` → SQLite |
| 12 | Should workers receive API keys or route through orchestrator proxy? | Direct forwarding via `PROVIDER_ENV_KEYS` in `buildWorkerEnv()` | Compromised worker gains all LLM provider credentials (A6 violation) | ⚠️ **Open** — Expert consensus: Orchestrator LLM proxy with feature flag for migration |
| 13 | How should approach strings be normalized for Evolution Engine clustering? | Exact string match in `groupByApproach()` | Pattern mining frequency analysis is meaningless — learning signal destroyed | ⚠️ **Open** — Proposed: token-based Jaccard similarity (threshold 0.6), no embedding model needed |
| 14 | Should EventBus split into governance (sync) vs telemetry (async)? | Single synchronous bus for all 41+ event types | Telemetry handlers block L0's 100ms budget with I/O | ⚠️ **Open** — 16 governance events (sync) vs 25+ telemetry events (async via `queueMicrotask`) |
| 15 | What formal grammar should `falsifiable_by` use? | Freeform `string[]` | No parser, no interop, ecosystem fragmentation | ⚠️ **Open** — Proposed: `{scope}:{target}:{event}` structured format. See ECP spec Appendix D |
| 16 | Should `confidence` separate tier reliability from engine certainty? | Single `confidence: number` in [0,1] | Conflates "deterministic output" with "60% sure" — aggregation logic compares apples and oranges | ⚠️ **Open** — Proposed: split to `tier_reliability` + `engine_certainty`. Breaking protocol change |
| 17 | What SWE-bench score threshold validates production readiness? | No external benchmark — 1,339 internal tests only | Cannot prove value to external adopters or compare against competitors | ⚠️ **Open** — Run SWE-bench-lite, establish baseline |
| 18 | Should Vinyan ship as a Claude Code hooks package first? | Standalone agent is the primary distribution | Adoption friction "Very High" — no users, no traces, no Evolution Engine activation | ⚠️ **Open** — `npx vinyan-hooks init` as lowest-friction adoption path |
| 19 | What is the maximum tolerable L0 handler latency? | 100ms budget (TDD §6) | Sync EventBus with 31 emit calls may exceed budget | ⚠️ **Open** — Depends on Q14 (EventBus split) resolution |
| 20 | Should shadow runner have OS-level isolation? | `cp -r` + `sh -c` as same user, no sandbox | Malicious test file reads parent filesystem, accesses network, exfiltrates data | ⚠️ **Open** — Apply same Docker flags as container dispatch: `--network=none`, `--cap-drop=ALL` |
| 21 | How should perception feed into Self-Model predictions? | Self-Model predicts from historical averages only, perception goes to prompt | Misses per-task signals (type errors present, low coverage, high blast radius) | ⚠️ **Open** — Proposed: feature-conditioned modifiers in `CalibratedSelfModel.predict()` |
| 22 | Is single-writer World Graph a bottleneck for concurrent tasks? | Single writer (Orchestrator), `maxParallelSubtasks: 4` | SQLite WAL serializes writes under contention, inflating L3 latency | ⚠️ **Open** — Related to Q2 (Phase 5 multi-writer). May also affect Phase 1-4 with parallel subtasks |
...
>
> **Dependency:** §20 (Migration) ← all Phase 5 components. §21 (Plugin System) ← §20. §22 (API + Session) ← §20. §23 (Coordinator) ← §22 + [a2a-protocol.md](a2a-protocol.md).
...
> → implementation-plan PH5.8, [a2a-protocol.md](a2a-protocol.md) | Axiom: A3 (advisory coordination), A1 (cross-instance: A generates, B verifies), A6 (delegated results re-verified locally)
...
Trust level determines allowed operations (see [a2a-protocol.md](a2a-protocol.md) §4.4).
