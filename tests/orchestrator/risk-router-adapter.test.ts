...
describe("RiskRouterImpl", () => {
  ...
  test("high blast radius escalates to L1+", async () => {
    ...
    const decision = await router.assessInitialLevel(
      makeInput({ targetFiles: ["src/core.ts"] }),
    );
  ...
  test("MIN_ROUTING_LEVEL:2 forces minimum L2", async () => {
    const router = new RiskRouterImpl(mockDepVerify(0));
    const decision = await router.assessInitialLevel(
      makeInput({ constraints: ["MIN_ROUTING_LEVEL:2"] }),
    );
    expect(decision.level).toBeGreaterThanOrEqual(2);
  });

  test("dep-verify failure uses conservative default", async () => {
    const failingVerify = async () => { throw new Error("dep-oracle crash"); };
    const router = new RiskRouterImpl(failingVerify as any);
    const decision = await router.assessInitialLevel(
      makeInput({ targetFiles: ["src/foo.ts"] }),
    );
    // Should not throw, returns a valid decision
  ...
  test("returns valid RoutingDecision shape", async () => {
    ...
    expect(decision).toHaveProperty("latencyBudget_ms");
  });
});
