...
| :--- | :--- | :--- | :--- |
| **A1** | **Epistemic Separation** | Generation and verification are performed by different components at every layer. No engine evaluates its own output. Three-layer scope: **(1) Understanding** — goal interpretation (`TaskUnderstanding`) is verified by the Goal Alignment Oracle, ensuring output matches user intent; **(2) Planning** — DAG creation (`TaskDecomposer`) is validated by deterministic structural criteria (5 machine-checkable rules); **(3) Execution** — code generation (Worker) is verified by the Oracle Gate (AST, type, dep, lint, test oracles). This is the foundational principle — the single insight that justifies Vinyan's existence. | §3 Reasoning Engines, §6 Truth Maintenance, §7 Mutation Protocol, Goal Alignment Oracle |
...
| **A5** | **Tiered Trust** | Deterministic evidence (compiler, type checker, tests) outranks heuristic evidence (complexity metrics), which outranks probabilistic evidence (LLM reasoning). Contradictions resolve by evidence tier — not by vote or LLM arbitration. Confidence traces record whether oracle verdicts were composed from independent or shared evidence. | §3.1 Tiered Registry, §3.2 Contradiction Resolution |
...
**Validation status (updated Phase 6+)**: All seven axioms are **proven in implementation**. A1 Execution and Planning layers were proven in Phase 0 (100% structural error reduction in A/B experiment); A1 Understanding layer is newly implemented via the Goal Alignment Oracle (heuristic tier, informational-only). A2 is **fully active** — `OracleVerdict` carries continuous confidence, `type: 'uncertain'|'contradictory'` states, evidence chains with temporal context, and Subjective Logic opinions. A3 is **fully proven** — `risk-router.ts` calculates risk scores and `calculateRoutingLevel()` deterministically routes L0–L3; no LLM in governance path. A5 is **fully proven** — `tier-clamp.ts` enforces three-tier confidence capping (engine tier, transport tier, peer trust) with `clampFull()` composing all three, and `ExecutionTrace.oracleIndependence` records the composition method, primary/corroborating oracle roles, and shared-evidence warnings. A7 is **fully proven** — `self-model.ts` computes per-task-type EMA prediction error with adaptive learning rate, sliding window miscalibration detection, and feeds back into routing calibration. Test suite: 400+ tests across all modules.

#### 1.1.1 Proposed Core Extensions (RFC, staged)

Three additional invariants are under evaluation as core axioms. They are documented here so design work can reference them, but they are **not yet binding contracts**. Promotion to A8–A10 official requires landing the minimum runtime slice for each.

| # | Proposed Axiom | Principle | Promotion Gate |
|:---|:---------------|:----------|:---------------|
| **A8** | **Traceable Accountability** | Every governance decision, action, and verdict must be replayable from evidence, provenance link (PROV-style `wasDerivedFrom` / `wasGeneratedBy` / `attributedTo`), actor identity, policy/router version, and timestamp. Decision-level provenance, not just file-level hashes. Extends A4 from artifact provenance to decision provenance. | Trace-store schema extension persisting provenance fields end-to-end; phase-learn populates them; round-trip test proves replayability. |
| **A9** | **Resilient Degradation** | Component failure (oracle unavailable, LLM provider failure, tool timeout, rate limit, peer unreachable, trace-store write failure) must degrade capability — not corrupt state, not cascade, not fail silently. Fallback paths, circuit breakers, retries, and SLO/error-budget behavior are **governance contracts** declared per failure class, not ad-hoc per-call defenses. Aligns with A2 by requiring controlled `unknown`/downgraded states instead of fabricated success. | Declared degradation strategy module mapping failure class → fallback; structured degradation events on the EventBus; metrics surfaced; tests cover oracle / provider / tool failure paths. |
| **A10** | **Goal-and-Time Grounding** | Every execution phase must remain bound to (a) the root user intent and (b) evidence freshness. Drift from intent or stale evidence must downgrade confidence, trigger re-grounding via the Goal Alignment Oracle, or request clarification — never silently proceed. Closes the loop between A1 Understanding and long-running execution. | Mid-execution re-grounding hooks at phase boundaries on high-risk/long-running tasks; freshness checks via existing temporal-decay; trace fields (under A8) record re-grounding attempts. |
...
**Status as of 2026-04**: Documented. A8 persistence MVP has landed (`governanceProvenance` trace envelope + SQLite round-trip), risk-router decisions populate learn-phase traces, and routed/short-circuit traces now carry governance provenance where they bypass Learn. A9 now has a runtime degradation event contract with metrics for normalized failure classes; provenance-bearing trace persistence failures fail closed instead of silently returning success, and tool/drafting failures are normalized through the same degradation bridge. A10 now records phase-boundary root-goal and temporal freshness grounding checks into traces for high-risk or long-running tasks; initial enforcement now pauses for goal-drift clarification and downgrades trace confidence on stale or low-confidence temporal facts. Broader A9 enforcement coverage and A10 re-grounding policy coverage are still pending. The official axiom set remains A1–A7.
---
