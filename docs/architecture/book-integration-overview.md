...
## 13. Wave 5 Partial — Shipped 2026-04-15

> **Status:** 3 of 9 Wave 5 backlog items + 1 Phase A §7 seam shipped.
> See Phase B §6/§7 for the file tables.
...
**What W5.1 actually removes:**
1. `(input as unknown as { riskScore?: number }).riskScore = routing.riskScore;` in `core-loop.ts`
2. `const riskScore = (task as unknown as { riskScore?: number }).riskScore;` in `debate-mode.ts`

Replaced by a typed `CriticContext` argument threaded through
`CriticEngine.review()`. Baseline critic + 3-seat debate both accept
the arg with `_context` prefix (ignore it); the router reads
`context.riskScore` directly. The interface is backward-compatible —
existing callers that pass no context get `undefined` and the router
defaults to baseline, matching the pre-Wave-5.1 behavior when no risk
signal was available.

**What `critic:debate_fired` enables:**
Dashboards and the Economy OS can now subscribe to a single event to
track when architecture debate fires, carrying `taskId`, `riskScore`,
`routingLevel`, and a `trigger` discriminator ('force' / 'risk-threshold').
This is the observability foundation for the full per-day cost cap
(Wave 5 backlog #7) once someone wires it to `CostLedger`.

**Remaining Wave 5 backlog** (6 items): W4.4 Implementation Team,
ScheduleWakeup, Fleet-live overview, Research Swarm Wave-2, Bus event
typed registry, Debate cost cap (full Economy OS integration),
TerminationSentinel class extraction. See Phase B §6 for current
status and blockers on each.
