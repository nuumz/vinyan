...
describe("attachTraceListener", () => {
  ...
  test("computes running average of qualityScore composite", () => {
    ...
    bus.emit("trace:record", {
      trace: makeTrace({
        qualityScore: { architecturalCompliance: 1, efficiency: 1, composite: 0.8, dimensions_available: 2, phase: "phase0" },
      }),
    });
    bus.emit("trace:record", {
      trace: makeTrace({
        qualityScore: { architecturalCompliance: 0.5, efficiency: 0.5, composite: 0.4, dimensions_available: 2, phase: "phase0" },
      }),
    });
  ...
  test("computes average duration", () => {
    ...
    bus.emit("trace:record", { trace: makeTrace({ duration_ms: 100 }) });
    bus.emit("trace:record", { trace: makeTrace({ duration_ms: 300 }) });

    expect(getMetrics().avgDuration_ms).toBeCloseTo(200, 5);
  });
