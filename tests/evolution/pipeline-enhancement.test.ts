...
import type { ExtractedPattern, EvolutionaryRule } from "../../src/orchestrator/types.ts";
...
describe("PH3.3: Evolution Pipeline Enhancement", () => {
  ...
  describe("Fix 4: Multi-condition rules", () => {
    test("escalation rule includes oracle_name when pattern provides it", () => {
      const rule = generateRule(makeAntiPattern({ oracleName: "type-checker" }))!;
      expect(rule.condition.oracle_name).toBe("type-checker");
      expect(rule.specificity).toBeGreaterThan(0);
    });

    test("escalation rule includes risk_above when pattern provides it", () => {
      const rule = generateRule(makeAntiPattern({ riskAbove: 0.5 }))!;
      expect(rule.condition.risk_above).toBe(0.5);
    });

    test("escalation rule includes model_pattern when pattern provides it", () => {
      const rule = generateRule(makeAntiPattern({ modelPattern: "claude-haiku" }))!;
      expect(rule.condition.model_pattern).toBe("claude-haiku");
    });
    ...
    test("preference rule includes multi-conditions", () => {
      ...
      expect(rule.condition.risk_above).toBe(0.3);
    });

    test("no extra conditions when pattern has no oracle/risk/model data", () => {
      const rule = generateRule(makeAntiPattern())!;
      expect(rule.condition.oracle_name).toBeUndefined();
      expect(rule.condition.risk_above).toBeUndefined();
      expect(rule.condition.model_pattern).toBeUndefined();
    });
  });
});
