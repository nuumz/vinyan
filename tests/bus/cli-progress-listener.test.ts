...
describe("attachCLIProgressListener", () => {
  test("task:start produces expected output", () => {
    ...
    bus.emit("task:start", {
      input: { id: "t-1", source: "cli", goal: "fix bug", budget: { maxTokens: 100, maxDurationMs: 1000, maxRetries: 1 } },
      routing: { level: 1, model: "mock/fast", budgetTokens: 100, latencyBudget_ms: 1000 },
    });
  ...
  test("oracle:verdict shown only in verbose mode", () => {
    ...
    bus.emit("oracle:verdict", {
      taskId: "t-1",
      oracleName: "type",
      verdict: { verified: false, confidence: 0.9, evidence: [], reason: "type error", type: "known" as const, fileHashes: {}, duration_ms: 10 },
    });
