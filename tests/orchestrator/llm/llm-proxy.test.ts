...
import { createProxyProvider, type LLMProxyServer, startLLMProxy } from '../../../src/orchestrator/llm/llm-proxy.ts';
...
describe('LLM Proxy — A6 Credential Isolation', () => {
  ...
  test('proxy returns error when no provider available', async () => {
    ...
    const provider = createProxyProvider(proxy.socketPath, 'balanced');
    await expect(provider.generate({ systemPrompt: 'test', userPrompt: 'test', maxTokens: 100 })).rejects.toThrow(
