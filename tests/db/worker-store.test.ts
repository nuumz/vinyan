...
function insertTrace(db: Database, workerId: string, opts?: {
  ...
  db.run(
    ...
    [
      id, `task-${id}`, Date.now(), 1, "test approach", "claude-sonnet",
      opts?.tokensConsumed ?? 1000, opts?.durationMs ?? 5000,
      opts?.outcome ?? "success", "{}", "[]",
      workerId, opts?.qualityComposite ?? 0.8,
...
describe("WorkerStore", () => {
  ...
  describe("stats computation from traces", () => {
    ...
    test("computes stats from execution_traces", () => {
      ...
      expect(stats.avgDuration_ms).toBeCloseTo((5000 + 3000 + 1000) / 3, 0);
