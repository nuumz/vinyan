...
describe('RuleStore', () => {
  test('insert and find active', () => {
    ...
    expect(active[0]!.condition).toEqual({ filePattern: '*.ts' });
  });

  test('findMatching filters by context', () => {
    store.insert(makeRule({ id: 'r1', condition: { filePattern: '*.ts' } }));
    store.insert(makeRule({ id: 'r2', condition: { filePattern: '*.py' } }));
