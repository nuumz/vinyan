...
## 13. Wave 5 — Shipped 2026-04-15 (phases 1, 2, and 3)

> **Status at end of phase 3:** 7 of 9 Wave 5 backlog items + 2 of 3
> Phase A §7 seams shipped (Wave 5 backlog #7 is now fully closed; a
> new W5.11 command has been added for Ch13 `maw costs` coverage).
> See Phase B §6, §7a, and §7b for file tables.

### Phase 1 (2026-04-15 earlier commit `813a4b9`)

| Item | Source | Effort | Status |
|------|--------|--------|--------|
| **W5.1** CriticEngine context object | Wave 5 backlog #5 + Phase A §7 seam #1 | Low — ~60 LOC | ✅ Shipped |
| **W5.4** `sentinelMaxNoopCycles` constructor option | Wave 5 backlog #8 | Trivial — ~15 LOC | ✅ Shipped |
| **W5 observability** `critic:debate_fired` bus event | Partial of Wave 5 backlog #7 (cost cap) | Low — ~30 LOC | ✅ Shipped |

**What W5.1 removes:** both `(task as unknown as { riskScore? }).riskScore` casts — replaced by a typed `CriticContext` argument threaded through `CriticEngine.review()`.

### Phase 2 (this commit)

| Item | Source | Effort | Status |
|------|--------|--------|--------|
| **W5.7a** Per-task `DebateBudgetGuard` | Phase 2 partial of Wave 5 backlog #7 | Medium — ~180 LOC + 14 tests | ✅ Shipped |
| **W5.2** `TaskDAG.preamble` + centralized merge | Phase A §7 seam #2 closure | Medium — ~120 LOC across 7 files + 6 tests | ✅ Shipped |

**What W5.7a enables:**
A new `DebateBudgetGuard` class tracks debate invocations per task id.
`DebateRouterCritic` consults the guard before firing the 3-seat
debate; when the cap is reached, the router emits `critic:debate_denied`
and falls through to the baseline critic. The factory wires a default
`maxPerTask: 1` which can be overridden via `OrchestratorConfig.debateMaxPerTask`.
Setting `maxPerTask: 0` disables debate entirely. This closes the
per-task axis of the Wave 5 backlog #7 cost cap; the per-day axis at
the Economy OS layer is still future work.

**What W5.2 fixes that was genuinely broken:**
The previous research-swarm preset mutated `input.constraints` inside
the decomposer (Phase A §7 seam #2). Analysis during phase 2 revealed
the mutation was **also a decorative no-op** with respect to the worker:
`understanding.constraints` is pre-computed in `prepareExecution()`
BEFORE the decomposer runs, so the worker's prompt assembler never
saw the added REPORT_CONTRACT. Wave 5.2 fixes both problems:

1. The decomposer no longer mutates — caller's input is clean
2. The preamble is threaded on the DAG (`plan.preamble`)
3. Phase-plan materializes an `enhancedInput` clone that subsequent
   phases see via a `ctx.input` swap
4. Agent-loop merges `plan.preamble` into the init turn's
   `understanding.constraints` just before sending to the worker

This is the **first time** the research-swarm REPORT_CONTRACT actually
reaches the LLM's system prompt.

### Phase 3 (this commit)

| Item | Source | Effort | Status |
|------|--------|--------|--------|
| **W5.7b** `DebateBudgetGuard.maxPerDay` + `whyDenied` discriminator | Completes Wave 5 backlog #7 (per-day axis) | Medium — ~130 LOC + 10 tests | ✅ Shipped |
| **W5.11** `vinyan tui costs` subcommand | NEW — closes Ch13 `maw costs` operator surface | Medium — ~130 LOC + 4 tests | ✅ Shipped |

**What W5.7b closes:**
With phase 2's per-task cap already in place, phase 3 adds the
per-day axis: a rolling counter of debate fires since midnight UTC
that the guard consults alongside the per-task counter. `whyDenied`
discriminates the failure so `critic:debate_denied` carries a precise
reason (`per-task debate cap reached` vs `per-day debate cap reached`).
Day rollover is handled via an injectable `now` clock + automatic
`pruneStaleFires` on every touch. This is a **guard-local** cap; an
operator that wants a USD-denominated daily limit should subscribe to
`critic:debate_fired` from the Economy OS layer and wire the cap there.

**What W5.11 adds:**
A new `vinyan tui costs` subcommand renders a one-shot snapshot of the
`CostLedger`:
...
Opens `${workspace}/.vinyan/vinyan.db` read-only without instantiating
a full orchestrator. Gracefully falls back to a friendly notice when
the DB or `cost_ledger` table is missing. This closes the `maw costs`
surface from Ch13 / Appendix A of the book.

### Cumulative Wave 5 status

| Shipped | Backlog # | Item |
|---|---|---|
| ✅ phase 1 | #5 | CriticEngine context object |
| ✅ phase 1 | #8 | Sentinel constructor option |
| ✅ phase 1 | #7 (part) | `critic:debate_fired` observability |
| ✅ phase 2 | #7 (part) | Per-task `DebateBudgetGuard` |
| ✅ phase 2 | Phase A seam #2 | `TaskDAG.preamble` + centralized merge |
| ✅ phase 3 | #7 (remainder) | Per-day `DebateBudgetGuard.maxPerDay` cap |
| ✅ phase 3 | NEW W5.11 | `vinyan tui costs` command (Ch13 / App A) |
| ⏸ deferred | #1 | W4.4 Implementation Team preset |
| ⏸ deferred | #2 | `ScheduleWakeup` primitive |
| ⏸ deferred | #3 | Fleet-live overview view |
