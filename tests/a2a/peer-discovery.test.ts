...
import { fetchAgentCard, discoverPeers, filterVinyanPeers } from "../../src/a2a/peer-discovery.ts";
...
function makeVinyanCard(overrides: Partial<A2AAgentCard> = {}): A2AAgentCard {
  return makeAgentCard({
    ...
    "x-vinyan-ecp": {
      ...
      oracle_capabilities: [
        { name: "ast-oracle", tier: "deterministic", languages: ["typescript"] },
      ],
...
describe("discoverPeers", () => {
  ...
  test("returns empty array when all peers unreachable", async () => {
    const peers = await discoverPeers(
      ["http://localhost:19996", "http://localhost:19995"],
      { fetchTimeoutMs: 500 },
    );
    expect(peers).toHaveLength(0);
  });
