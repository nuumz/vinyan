...
describe('MCPClientPool', () => {
  test('constructor accepts config list', () => {
    const pool = new MCPClientPool([{ name: 'test-server', command: 'echo hello', trustLevel: 'local' }]);
    expect(pool.size).toBe(0); // not connected yet
  });

  test('listServers returns empty before initialize', () => {
    const pool = new MCPClientPool([{ name: 'server-a', command: 'echo a', trustLevel: 'local' }]);
  ...
  test('callToolVerified throws for unknown server', async () => {
    ...
    await expect(pool.callToolVerified('nonexistent', 'tool', {}, mockGate, '/tmp')).rejects.toThrow(
  ...
  test('initialize handles connection failure gracefully', async () => {
    ...
    const pool = new MCPClientPool([{ name: 'bad-server', command: 'nonexistent-binary-xyz', trustLevel: 'local' }]);
