...
import { TRACE_SCHEMA_SQL, SELF_MODEL_PARAMS_SCHEMA_SQL } from "../../src/db/trace-schema.ts";
...
describe("PH3.8: Phase 3 Integration Validation", () => {
    ...
    test("accuracy improves with consistent calibration data", async () => {
      ...
      for (let i = 0; i < 20; i++) {
        const pred = await model.predict(
          { id: `t-${i}`, source: "cli", goal: "refactor auth", budget: { maxTokens: 1000, maxDurationMs: 30000, maxRetries: 3 } },
          perception,
        );
        ...
        const trace: ExecutionTrace = makeTrace({
          ...
          outcome: "success",
          qualityScore: { composite: 0.75, architecturalCompliance: 0.8, efficiency: 0.7, dimensions_available: 2, phase: "phase0" },
          duration_ms: 1000,
        });
    ...
    test("skill created from success pattern, promoted after outcomes, demoted on failure", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        stores.traceStore.insert(makeTrace({
          ...
          outcome: "success",
          qualityScore: { composite: 0.85, architecturalCompliance: 0.9, efficiency: 0.8, dimensions_available: 2, phase: "phase0" },
          session_id: `s-${i % 5}`,
      ...
      for (let i = 0; i < 10; i++) {
        stores.traceStore.insert(makeTrace({
          ...
          approach: "inline-all",
          outcome: "success",
          qualityScore: { composite: 0.45, architecturalCompliance: 0.5, efficiency: 0.4, dimensions_available: 2, phase: "phase0" },
          session_id: `s-${i % 5}`,
          affected_files: ["src/auth.ts"],
        }));
      }
  ...
  describe("Graceful degradation", () => {
    ...
    test("cold start: data gate blocks sleep cycle with empty DB", async () => {
      ...
      const runner = new SleepCycleRunner({
        traceStore, patternStore, ruleStore,
