...
function makeWorkerOutput(overrides?: Partial<WorkerOutput>): WorkerOutput {
  return {
    taskId: "task-001",
    proposedMutations: [
      { file: "src/foo.ts", content: "const x = 1;", explanation: "Fixed type" },
    ],
    proposedToolCalls: [],
    uncertainties: [],
    tokensConsumed: 500,
...
describe("ToolResultSchema", () => {
  test("success result → parses", () => {
    const result: ToolResult = { callId: "tc-1", tool: "write_file", status: "success", duration_ms: 50 };
    expect(ToolResultSchema.parse(result)).toEqual(result);
  });

  test("error result → parses", () => {
    const result: ToolResult = { callId: "tc-1", tool: "write_file", status: "error", error: "ENOENT", duration_ms: 10 };
    expect(ToolResultSchema.parse(result)).toEqual(result);
  });
  ...
  test("invalid status → throws", () => {
    expect(() => ToolResultSchema.parse({ callId: "x", tool: "x", status: "unknown", duration_ms: 0 })).toThrow();
  });
});
...
describe("TaskInputSchema", () => {
  ...
  test("invalid source → throws", () => {
    expect(() =>
      TaskInputSchema.parse({ id: "x", source: "unknown", goal: "x", budget: { maxTokens: 1, maxDurationMs: 1, maxRetries: 1 } }),
    ).toThrow();
  });
});
...
describe("WorkingMemoryStateSchema", () => {
  ...
  test("populated memory → parses", () => {
    const mem = makeWorkingMemory({
      failedApproaches: [{ approach: "inline function", oracleVerdict: "type: type error at line 5", timestamp: Date.now() }],
  ...
  test("valid DAG → parses", () => {
    const dag: TaskDAG = {
      nodes: [
        ...
        { id: "n2", description: "Step 2", targetFiles: ["b.ts"], dependencies: ["n1"], assignedOracles: ["type", "dep"] },
      ],
    };
...
describe("WorkerInputSchema", () => {
  test("valid complete input → parses", () => {
    const input = makeWorkerInput({
      plan: { nodes: [{ id: "n1", description: "Step 1", targetFiles: ["a.ts"], dependencies: [], assignedOracles: ["ast"] }] },
    });
