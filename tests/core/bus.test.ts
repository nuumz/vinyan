...
import { createBus, EventBus, type VinyanBus } from '../../src/core/bus.ts';
...
describe('EventBus', () => {
  test('on/emit delivers payload synchronously', () => {
    ...
    const payload = {
      input: makeInput(),
      routing: { level: 0 as const, model: 'test', budgetTokens: 100, latencyBudgetMs: 1000 },
    };
...
function makeRouting() {
  return { level: 0 as const, model: 'test', budgetTokens: 100, latencyBudgetMs: 1000 };
}
