...
import type { TaskInput, TaskResult, WorkerSelectionResult, RoutingDecision } from "../../src/orchestrator/types.ts";
    ...
    const deps = buildBaseDeps({
      ...
      shadowRunner: {
        enqueue: (taskId: string) => {
          ...
          return { id: "sj-1", taskId, status: "pending" as const, enqueuedAt: Date.now(), retryCount: 0, maxRetries: 1 };
        },
      } as any,
      riskRouter: {
        assessInitialLevel: async () => ({
  ...
  test("uncertain worker selection short-circuits dispatch", async () => {
    ...
    const deps = buildBaseDeps({
      ...
      workerPool: {
        dispatch: async () => {
          dispatchCalled = true;
          return { mutations: [], proposedToolCalls: [], tokensConsumed: 0, duration_ms: 0 };
        },
      },
      riskRouter: {
        assessInitialLevel: async () => ({
